...
(() => {
  ...
  async function fsListTestTemplates(plan){
    ...
    for await (const [name, handle] of templatesDir.entries()){
      ...
      const title = (testData?.test_name || "").trim() || name.replace(/\.json$/i, "");
...
`;
              ...
              try{
                ...
                const resp = await fetch("https://api.openai.com/v1/responses", {
                  ...
                  body: JSON.stringify({
                    model,
                    input: [
                      { role:"system", content:"Return ONLY the field value text. No JSON, no extra words, no markdown. Use field_prompt and field_label as guidance. Use authoritative_values to keep consistency across tests. Do NOT repeat TODO or Auto-filled text; replace it with improved content even if brief." },
                ...
                if(val && val !== String(t.data[field] ?? "").trim()){
                  t.data[field] = val;
  ...
  async function aiGeneratePlan(inputs, basePlan){
    const key = getAiKey();
    const model = state.prefs.ai_model || "gpt-4.1-mini";
    const payload = {
      inputs,
      base_template: basePlan ? {
        name: basePlan.name,
        tests: (basePlan.tests || []).map(t => t.data || t)
      } : null,
      schema_fields: Object.keys(TEST_SCHEMA_DEFAULT).filter(k => k !== "_ai"),
      notes: "Return JSON only. Leave fields blank if unknown. Provide a tests array."
    };
    try{
      ...
      const resp = await fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
          model,
          input: [
            { role:"system", content:"Return STRICT JSON with shape: {\"tests\":[{...}]}. Use schema_fields only. Leave unknown fields empty strings. Use inputs to build a plan structure. If base_template is provided, align style and naming. Do not include extra keys." },
            { role:"user", content: JSON.stringify(payload, null, 2) }
          ],
      ...
      const data = await resp.json();
      const raw = extractResponseText(data);
      const parsed = safeParseJson(raw, "AI Create");
  ...
  function buildPlanFromAi(name, aiPlan, testCount){
    const list = Array.isArray(aiPlan?.tests) ? aiPlan.tests : [];
    const trimmed = list.slice(0, testCount);
    while(trimmed.length < testCount) trimmed.push({});
    const tests = trimmed.map((d, idx) => {
      const raw = d || {};
      const data = normalizeTestData(raw);
      for(const k of ["ll","tv","ul","units"]){
        if(raw[k] == null || String(raw[k]).trim() === "") data[k] = "";
      }
      return {
        id: uid(),
        order: idx + 1,
  ...
  function renderTestsList(){
    ...
    const listTitle = $("testListTitle");
    if(listTitle) listTitle.textContent = "Test List";
        ...
        const items = [
          {
            ...
            onClick: async () => {
              if(!state.planRootHandle){
                toast("Templates", "Set a Plan Folder to use test templates.", "warn");
                return;
              }
              const name = prompt("Template name:", t.data?.test_name || "New Test Template");
              if(name === null) return;
          ...
          ,{
            ...
            onClick: async () => {
              ...
              plan.tests.push(copy);
              const resorted = (plan.tests || []).slice().sort((a,b) => (a.order||0)-(b.order||0));
              resorted.forEach((item, i) => item.order = i + 1);
              state.currentTestId = copy.id;
              renderEditor();
              logLine("Duplicated test (inserted below).", "ok");
              toast("Test duplicated", "Inserted below.", "ok");
            }
          },{
            id: "delete_test",
            label: "Delete",
            onClick: () => {
              ...
              plan.tests = plan.tests.filter(x => x.id !== t.id);
              if(state.currentTestId === t.id){
  ...
  function renderForm(){
    ...
    if(saveBtn) saveBtn.textContent = state.currentTestTemplate ? "Save Template" : "Save Test";
    const newBtn = $("btnNewTest");
    if(newBtn) newBtn.textContent = state.currentTestTemplate ? "Tests" : "New Test";
  ...
  function createNewPlanFlow(){
    showModal("Create New Plan", (body, footer) => {
      ...
      body.innerHTML = `
        ...
        <input class="input" id="newPlanName" placeholder="e.g., Power Supply CCA (Generic)" />
        <div class="hint">Slug rules: lowercase, spaces???underscores, remove non [a-z0-9_], collapse underscores.</div>

        <div class="hr"></div>
        <div class="label">Use Plan Template (optional)</div>
        <select class="select" id="newPlanTemplate">
        ...
        <div class="row">
          <div>
            ...
            <select class="select" id="newPlanAiMode">
              <option value="off">Off</option>
              <option value="on">On (structure only)</option>
            </select>
            <div class="hint">Model: <span class="tag">${escapeHtml(model)}</span></div>
          </div>
          <div>
            <div class="label"># of Tests</div>
            <input class="input" id="newPlanTestCount" type="number" min="1" step="1" value="5" />
          </div>
        </div>
        ...
        <div class="row">
          <div>
            <div class="label">UUT P/N</div>
            <input class="input" id="newPlanUutPn" placeholder="e.g., TBD or 21A-00" />
          </div>
          <div>
            <div class="label">Device Type</div>
            <input class="input" id="newPlanDeviceType" placeholder="e.g., Power Supply, CCA, Sensor Module" />
          </div>
        </div>
        <div class="label" style="margin-top:10px;">Test Scope</div>
        <textarea class="input" id="newPlanScope" placeholder="High-level scope and exclusions"></textarea>
        <div class="row" style="margin-top:10px;">
          <div>
            <div class="label">Test Types</div>
            <input class="input" id="newPlanTestTypes" placeholder="e.g., Inspection, Functional Verification" />
          </div>
          <div>
            <div class="label">Target Standards</div>
            <input class="input" id="newPlanStandards" placeholder="e.g., IPC-A-610, MIL-STD-461" />
          </div>
        </div>
        <div class="row" style="margin-top:10px;">
          <div>
            <div class="label">Environment</div>
            <input class="input" id="newPlanEnvironment" placeholder="e.g., Ambient, 23?C ?? 5?C" />
          </div>
          <div>
            <div class="label">Pass/Fail Philosophy</div>
            <input class="input" id="newPlanPassFail" placeholder="e.g., Conservative, safety-first" />
          </div>
        </div>
      `;
      footer.innerHTML = `
      ...
      runAsync(async () => {
        const sel = $("newPlanTemplate");
        const templates = await Templates.listTemplates();
        sel.innerHTML = `<option value="">None (blank)</option>` + templates.map(t =>
      ...
      $("confirmNewPlan").onclick = async () => {
        ...
        const templateId = $("newPlanTemplate").value || "";
        const aiMode = $("newPlanAiMode").value || "off";
        const testCount = Math.max(1, Number($("newPlanTestCount").value) || 1);

        let basePlan = null;
        if(templateId){
          basePlan = await Templates.loadTemplate(templateId).catch(() => null);
        }

        if(aiMode === "on"){
          const key = getAiKey();
          if(!key){
            toast("AI Create", "Set an API key in Preferences first.", "warn");
            return;
          }
          const inputs = {
            plan_name: name,
            uut_pn: $("newPlanUutPn").value.trim(),
            device_type: $("newPlanDeviceType").value.trim(),
            test_scope: $("newPlanScope").value.trim(),
            test_types: $("newPlanTestTypes").value.trim(),
            target_standards: $("newPlanStandards").value.trim(),
            environment: $("newPlanEnvironment").value.trim(),
            pass_fail_philosophy: $("newPlanPassFail").value.trim(),
            test_count: testCount
          };
          const aiPlan = await aiGeneratePlan(inputs, basePlan);
          if(!aiPlan){
            toast("AI Create", "AI generation failed.", "warn");
            return;
          }
          const plan = buildPlanFromAi(name, aiPlan, testCount);
          await Store.savePlan(plan);
          hideModal();
          logLine(`Created plan: ${plan.name} (AI)`, "ok");
        ...
        if(basePlan){
          const copy = structuredClone(basePlan);
          copy.id = uid();
          copy.name = name;
          copy.slug = slugifyPlanName(name);
          copy.folder = null;
          copy.created = nowIsoDate();
          copy.updated = nowIsoDate();
          copy.userNotes = basePlan.userNotes ?? extractUserNotes(basePlan.markdown || "");
          copy.tests = (basePlan.tests || []).map((t, idx) => ({
            id: uid(),
            order: t.order ?? (idx + 1),
            modified: false,
            data: structuredClone(t.data || {})
          ...
          hideModal();
          logLine(`Created plan from template: ${copy.name}`, "ok");
          toast("Plan created", copy.name, "ok");
          runAsync(renderLibrary());
          return;
        }
  ...
  async function renamePlanFlowasync function renamePlanFlow(planId){
  ...
  async function newTest(){
    ...
    logLine(`Created new test (unsaved)${chosen?.name ? ` from "${chosen.name}"` : ""}.`, "ok");
    toast("New test", chosen?.name ? `Test created from "${chosen.name}".` : "Test created (unsaved).", "ok");
    renderEditor();
  }
  ...
  async function saveCurrentTest(){
    ...
    if(state.currentTestTemplate){
      ...
      if(!basePlan){
        toast("Templates", "Base plan not found.", "warn");
        return;
      }
      const templatesDir = await fsEnsureTestTemplatesDir(basePlan, true);
      if(!templatesDir){
        toast("Templates", "Plan folder not set or unavailable.", "warn");
        return;
      }
      ...
      $("dirtyHint").style.display = "none";
      renderEditor();
      logLine(`Saved test template: ${state.currentTestTemplate.file}`, "ok");
        ...
        <div class="row" style="margin-top:10px;">
          ...
          <div>
            ...
            <select class="select" id="prefNewTestTemplateDefault">
              <option value="__blank__">Blank (schema)</option>
            </select>
            <div class="hint">Templates are read from the current plan's tests/templates folder.</div>
          </div>
        </div>
  ...
  async function getAvailableTestTemplates(plan){
    const list = [{ id: "__blank__", name: "Blank (schema)", data: {} }];
  ...
  function pickTestTemplate(templates){
    return new Promise(resolve => {
      if(!templates || !templates.length){
        toast("Templates", "No test templates found.", "warn");
        return resolve(null);
      }
      const options = templates.map(t =>
        `<option value="${escapeHtml(t.id)}">${escapeHtml(t.name)}</option>`
      ).join("");
      showModal("Choose Test Template", (body, footer) => {
        body.innerHTML = `
          <div class="label">Template</div>
          <select class="select" id="testTemplatePick">${options}</select>
          <div class="hint">Templates are loaded from the plan's tests/templates folder.</div>
        `;
        footer.innerHTML = `
  ...
  async function openManageTestTemplates(){
    const plan = state.currentPlan;
    if(!plan){
      toast("Templates", "Open a plan first.", "warn");
      return;
    }
    if(!state.planRootHandle){
      toast("Templates", "Set a Plan Folder to use test templates.", "warn");
    ...
    showModal("Manage Test Templates", (body, footer) => {
      ...
      const rows = list.map(t => `
        <div class="item">
          ...
          <div class="actions">
            <button class="btn mini" data-edit="${escapeHtml(t.file)}">Edit</button>
      ...
      $("tmplCreateFromCurrent").onclick = async () => {
        const test = currentTest();
        if(!test){
          toast("Templates", "Select a test first.", "warn");
          return;
        }
        const name = prompt("Template name:", test.data?.test_name || "New Test Template");
        if(name === null) return;
      ...
      body.querySelectorAll("[data-edit]").forEach(btn => {
        btn.addEventListener("click", async () => {
          const file = btn.getAttribute("data-edit");
          if(!file) return;
  ...
  async function refreshAvailableModels(){
    ...
    function updateAiBadge(){
      ...
      $("aiText").textContent = hasKey ? "Ready" : "Set key in Preferences";
      updateAiUI();
    }
