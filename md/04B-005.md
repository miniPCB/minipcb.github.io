...
The collector load consists of **R4**, **R7**, and **R8**, with **R7** as a multiturn potentiometer to adjust gain or output bias. These resistors determine the voltage drop across the collector and directly affect the voltage gain.  

**TP9** and **TP10** are used to probe the collector and output signal path.
...
**TP7** and **TP8** enable measurement of the emitter voltage and overall emitter network behavior. Jumper **J1** allows the emitter network to be removed from the circuit.
...
| #  | Test                         | Test Method                              | Lower Limit | Target | Upper Limit |
|----|------------------------------|-------------------------------------------|-------------|--------|-------------|
| 1  | Supply current check         | Apply 9 V to V+, measure Icc              | 1 mA        | 2–3 mA | 10 mA       |
| 2  | Emitter voltage (VE)         | Measure DC at emitter (TP7, Output ref)   | 0.3 V       | 0.4–0.6 V | 0.8 V   |
| 3  | Base voltage (VB)            | Measure DC at transistor base (TP5)       | 0.7 V       | 0.9–1.0 V | 1.2 V   |
| 4  | Collector voltage (VC)       | Measure DC at collector/output (TP9/Out)  | 2 V         | 3–6 V  | 7 V         |
| 5  | Output DC bias               | Measure DC at Output pin                  | 2 V         | ~½ Vcc (≈ 4.5 V) | 7 V |
### Why these values?

- **Supply current**: With typical bias resistors, a small-signal BJT amplifier draws a few mA. Too low = open bias, too high = short or misbias.
- **VE**: Should be a few hundred mV above GND (set by emitter resistor).
- **VB**: Should be ~0.7 V higher than VE for a silicon BJT.
- **VC**: Should sit in mid-supply for maximum swing (not near 0 or V+).
- **Output DC bias**: Should track VC, since the collector is the output node.
...
- Use a **10 kΩ load** on the Output unless your board specifies otherwise.
...
2. **Bias Calibration (Set Quiescent Point)**  
   - Measure the **collector voltage** at TP9.
   - Adjust **R2** until the **collector voltage** is near **½ VCC (≈ 4–5 V)**.  
...
5. **Final Checks**  
   - Re-run Static Tests: VE, VB, VC within limits.  
